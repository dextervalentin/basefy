...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $action = (string)($_POST['action'] ?? '');
    ...
    if ($action === 'documentos_lote') {
        ...
        $uploaded = 0;
        $uploadErrors = [];
      $docStep = verifStatus($conn, $uid, 'documentos');
      $docStepObs = (string)($docStep['observacao'] ?? '');
      $docStepRejectedHint = (strtolower((string)($docStep['status'] ?? '')) === 'rejeitado') || (stripos($docStepObs, 'rejeitad') !== false);
        ...
        $locked = !$docStepRejectedHint && $hasFile && in_array($status, ['pendente', 'aprovado', 'verificado'], true);
            if ($locked) continue;
...
$dadosObs = (string)($vDados['observacao'] ?? '');
$dadosRejectedHint = strtolower((string)$vDados['status']) === 'rejeitado' || (stripos($dadosObs, 'rejeitad') !== false);
...
foreach ($docStatus as $s) {
  ...
  if ($docsRejectedHint || !$hasFile || $st === 'rejeitado') {
        $docUploadOpen = true;
        break;
    }
}
  ...
  <div class="space-y-3">
    <div class="bg-blackx2 border border-blackx3 rounded-2xl overflow-hidden" x-data="{ open: <?= $vDados['status'] !== 'verificado' ? 'true' : 'false' ?> }">
      ...
      <div x-show="open" x-transition x-cloak class="px-5 pb-5 border-t border-blackx3">
        <?php
          $dadosLocked = in_array($vDados['status'], ['verificado', 'pendente'], true) && !$dadosRejectedHint;
        ?>
        <form method="post" class="pt-4 space-y-3">
          ...
          <?php elseif ($vDados['status'] === 'pendente' && !$dadosRejectedHint): ?>
            <?php
            ...
            <?php endif; ?>
          <?php elseif ($vDados['status'] === 'rejeitado' || $dadosRejectedHint): ?>
          ...
          <form method="post" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-3">
              <?php foreach ($docTypes as $docKey => $meta):
                ...
                $canUpload = $docsRejectedHint || !$hasFile || $st === 'rejeitado';
              ?>
